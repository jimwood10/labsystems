...
The policy scripts, list-immutable.txt and makeimmutablelistpolicy.txt,  refer to fileset "worm". 
Change this to your actual fileset. Eg
sed -i 's/worm/testIam/g' makeimmutablelistpolicy.txt
...
Check the listing of list-immutable.txt  with:
mmapplypolicy fs1 -P list-immutable.txt -f ./gpfs -I defer
(the naming of this policy, imho, is counter-intuitive. It lists files that are not immutable)

cat gpfs.list.immut

Test makeimmutablelistpolicy 
mmapplypolicy fs1 -P makeimmutablelistpolicy.txt -N <node> -I test
...
mmapplypolicy fs1 -P makeimmutablelistpolicy.txt -N <node>
